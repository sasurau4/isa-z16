module Z16Decoder (
    input wire [15:0] i_instr,  // input instruction
    output wire [3:0] o_opcode,  // output opcode
    output wire [3:0] o_rd_addr,  // output destination register address
    output wire [3:0] o_rs1_addr,  // output source register 1 address
    output wire [3:0] o_rs2_addr,
    output wire [15:0] o_imm,  // output immediate value
    output wire o_rd_wen,  // output write enable for destination register
    output wire o_mem_wen,  // output write enable for memory
    output wire [3:0] o_alu_ctrl  // output ALU control signal
);
  ...
  // sign-extended immediate value based on opcode
  function [15:0] get_imm;
    input [15:0] i_instr;
    begin
      case (i_instr[3:0])
        // Syntax for concatenation: {{number of bits to extend}{sign bit}}, immediate bits
  ...
  function get_mem_wen;
    input [15:0] i_instr;
    begin
      // enable if the instruction is STORE
